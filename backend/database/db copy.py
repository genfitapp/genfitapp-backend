...
def populate_exercise_table(drop=True):
    ...
    json_path = Path(__file__).resolve().parent.parent.parent / 'algorithm' / 'dataset_4.json'
    ...
    for exercise in json_data:
        # Normalize fields and ensure correct types for PostgreSQL arrays
        data = [
            exercise.get('Exercise', ''),
            exercise.get('Main muscle(s)', []),
            exercise.get('Secondary muscles', []),
            'animation_path_placeholder',   # You can update with actual animation paths
            # 'Exercise Description',  # written_instructions_placeholder
            exercise.get('Exercise Description', ''),
            exercise.get('Movement', ''),
            exercise.get('Lower bound (lbs/resistance/time)', 0),

            # Make sure that the level is of type list before populating it in
            # the table.
            ...
            # Equipment is a list of lists (e.g., [["None"]]) → flatten
            # Needs to be flatten of should find other ways to store the lists.
            # One exapmle: " ".join each list in the list of list and make it a
            # list of string spearated by space.
            # [item for sublist in exercise.get('Equipment', []) for item in
            # sublist],
            ...
            # Loading type is BOOLEAN in your schema, convert string to bool
